...
test("Drag And Drop", async ({ page }) => {
  ...
  const dragFrame = page.frameLocator(".demo-frame");
  await dragFrame.locator("#draggable").dragTo(dragFrame.locator("#droppable"));
});

test("Drag And Drop Using Mouse", async ({ page }) => {
  await page.goto("https://jqueryui.com/droppable/");
  const dragFrame = page.frameLocator(".demo-frame");
  await dragFrame.locator("#draggable").hover();
  await page.mouse.down();
  await dragFrame.locator("#droppable").hover();
  await page.mouse.up();
});
